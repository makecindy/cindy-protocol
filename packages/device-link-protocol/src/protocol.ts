 ...
 *
 * 迁移备注:接线前,两侧各自持有一份等价副本
 * (packages/device-link/src/protocol.ts 的路由头子集 与
 *  apps/device-link-server/src/device-link/protocol.ts),靠注释约定人工同步;
 * 接线后两侧改为 import 本包,删除各自副本,PROTOCOL_VERSION 从此只有一处。
 */
