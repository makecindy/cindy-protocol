...
describe('round-trip: 每种消息构造后可被解析且等价', () => {
  ...
  it('task.dispatch 带来源元数据(source: im + threadContext + userText)', () => {
    ...
    expectReject(
      { ...base, payload: { ...base.payload, source: { im: 'slack', threadContext: [{ author: 'a' }] } } },
  ...
  it('session.archive', () => {
    ...
    // externalKey 必须非空
    expectReject(
      { ...msg, payload: { externalKey: '' } },
      'session.archive.externalKey',
    );
  });
...
describe('task.dispatch 字段约束', () => {
  ...
  it('sessionId 给了空串拒收', () => {
    expectReject(
      { ...valid, payload: { ...valid.payload, sessionId: '' } },
      'sessionId',
    );
  });
  ...
  it('attachments 校验: 非数组 / 元素缺字段 / 超上限', () => {
    ...
    expectReject(
      { ...valid, payload: { ...valid.payload, attachments: [{ name: 'a', mimeType: '' , dataBase64: 'x' }] } },
      'mimeType',
    );
    expectReject(
      { ...valid, payload: { ...valid.payload, attachments: [{ name: 'a', mimeType: 'image/png', dataBase64: '' }] } },
      'dataBase64',
    );
    expectReject(
      { ...valid, payload: { ...valid.payload, attachments: [{ name: 1, mimeType: 'image/png', dataBase64: 'x' }] } },
      'name',
    );
    const tooMany = Array.from({ length: 17 }, () => ({ name: null, mimeType: 'image/png', dataBase64: 'x' }));
...
describe('task.ack 三态联动约束', () => {
  ...
  it('rejected 必须带枚举内 reason 且 sessionId 为 null', () => {
    expectReject(
      { ...valid, payload: { ...valid.payload, result: 'rejected', reason: null, sessionId: null } },
      'reason must be one of',
    );
    expectReject(
      { ...valid, payload: { ...valid.payload, result: 'rejected', reason: 'nope', sessionId: null } },
      'reason must be one of',
    );
    expectReject(
      {
