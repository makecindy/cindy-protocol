...
const SNAPSHOT: BindStatePayload = {
  bindings: [
    { teamId: 'T0AAA', teamName: 'XD Inc.', slackUserId: 'U0123', slackUserName: 'cindy' },
...
describe('可选 teamId 字段族', () => {
  ...
  it('prefs.set.teamId / prefs.state 条目 teamId 可选', () => {
    roundTrip(makePrefsSet({ requestId: 'r-1', workspace: 'main', model: 'opus', teamId: 'T0AAA' }));
    roundTrip(
      makePrefsState({
        replyTo: null,
        bound: true,
  ...
  it('TaskSource.teamId / teamName 可选', () => {
    roundTrip(
      makeTaskDispatch({
        ...
        workspace: 'main',
        prompt: 'hi',
        source: { im: 'slack', channelName: '#general', teamId: 'T0AAA', teamName: 'XD Inc.' },
      }),
    );
