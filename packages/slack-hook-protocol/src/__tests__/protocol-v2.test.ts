...
const MODELS_RESPONSE_LEGACY: QueryResponsePayload = {
  ...
  agents: [
    {
      agentKind: 'codex',
      models: [{ id: 'gpt-5.5', label: 'GPT-5.5', efforts: ['low', 'high'], defaultEffort: 'high' }],
    },
  ],
};
...
describe('v2 新帧 round-trip', () => {
  ...
  it('bind.update 全状态', () => {
    ...
    roundTrip(makeBindUpdate({ state: 'none', slackUserId: null, slackUserName: null, message: null }));
    ...
    roundTrip(makeBindUpdate({ state: 'denied', slackUserId: null, slackUserName: null, message: null }));
    roundTrip(makeBindUpdate({ state: 'expired', slackUserId: null, slackUserName: null, message: null }));
    roundTrip(makeBindUpdate({ state: 'failed', slackUserId: null, slackUserName: null, message: '该 workspace 未安装本 app' }));
    ...
    roundTrip(makeBindUpdate({ state: 'revoked', slackUserId: null, slackUserName: null, message: '被新设备顶掉' }));
  });
  ...
  it('query.request / query.response(workspaces 与 models)', () => {
    ...
    roundTrip(
      makeQueryResponse({ queryId: 'q-1', kind: 'workspaces', ok: true, error: null, workspaces: ['cindy', 'blog'] }),
    );
    roundTrip(makeQueryResponse(MODELS_RESPONSE));
    roundTrip(makeQueryResponse(MODELS_RESPONSE_LEGACY));
    roundTrip(makeQueryResponse({ queryId: 'q-3', kind: 'models', ok: false, error: 'device busy' }));
  });
...
describe('bind 帧字段联动', () => {
  ...
  it('confirmed 必须带 slackUserId; pending 必须带 authorizeUrl', () => {
    ...
    expectReject(
      { ...msg, payload: { state: 'pending', slackUserId: null, slackUserName: null, message: null } },
    ...
    expectReject(
      {
        ...msg,
        payload: { state: 'confirmed', slackUserId: 'U1', slackUserName: null, message: null, authorizeUrl: 123 },
      },
  ...
  it('failed 必须带 message; 未知 state 拒收', () => {
    const msg = makeBindUpdate(CONFIRMED_UPDATE);
    expectReject(
      { ...msg, payload: { state: 'failed', slackUserId: null, slackUserName: null, message: null } },
      'message must be a non-empty string when state is failed',
    );
    // reason 形状校验: 非 string|null 拒收; 未知字符串取值放行(前向兼容)
    expectReject(
      { ...msg, payload: { state: 'failed', slackUserId: null, slackUserName: null, message: 'x', reason: 42 } },
    ...
    expect(
      parseHookMessage(
        JSON.stringify({
          ...msg,
          payload: { state: 'failed', slackUserId: null, slackUserName: null, message: 'x', reason: 'future-reason' },
        }),
      ).ok,
    ).toBe(true);
    expectReject(
...
describe('query 帧字段联动', () => {
  ...
  it('models 条目形状校验(efforts / defaultEffort)', () => {
    ...
    (bad2.agents as Array<{ models: Array<Record<string, unknown>> }>)[0].models[0].defaultEffort = 42;
  ...
  it('permissionModes 可选字段形状校验', () => {
    ...
    (badId.agents as Array<{ permissionModes: Array<Record<string, unknown>> }>)[0].permissionModes[0].id = '';
    ...
    delete (badLabel.agents as Array<{ permissionModes: Array<Record<string, unknown>> }>)[0].permissionModes[0].label;
    expectReject({ ...base, payload: badLabel }, 'permissionModes[0].label must be a non-empty string');
  });
...
describe('v1 帧扩展行为', () => {
  it('dispatch options.effort 合法透传, 非法拒收', () => {
    ...
    const msg = makeTaskDispatch({ requestId: 'r1', externalKey: 'C1:1.1', workspace: 'x', prompt: 'p' });
    expectReject(
