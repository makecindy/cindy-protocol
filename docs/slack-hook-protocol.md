...
| `provider.behavior.state` | server → desktop | 全量快照:三个全局字段是**已解析的** effective 非空枚举值(不是 patch,不接受 `null`/`undefined`);`groupActivation` 是 `{chatId: value}` 的完整 map(≤500 条)。`bound=false` 时字段联动(parse 强制)收敛为默认值 + 空 map                                                                                                                                                                                                                                                                                |
