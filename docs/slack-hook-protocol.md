...
- 收到一个**已经绑定到别的任务**的 `requestId` 时拒绝登记并忽略该帧,不得覆盖已有路由(见下面的 id 命名空间要求);
- **连接断开时必须收口该连接上所有"已 reopen 但还没收到 `turn.end`"的消息** —— 恢复成原来的终态,或改写成一句「续跑中断」。desktop 的续跑记账只在进程内,崩溃 / 重启后不会补发任何帧;没有这条清理,那些消息会永久停在假的"进行中"上,比本能力上线前更糟。
...
**已声明接受的降级**:本帧发出后、server 装上映射前连接断开时,映射不存在而后续帧被忽略,这一轮续跑的结果不回流 —— 退回"消息停在失败上"的现状(与本能力上线前一致),用户可在渠道重发。协议刻意不为此加 ack 往返:回流是增强而非关键路径。
